...
from src import mnist_models as models
import torch
...
class MnistAutoencoder(pl.LightningModule):
    def __init__(self,
        ...
        # mnist specific
        self.inverse_normalize = utils.inverse_normalization()
        ch = encoder_channels if self.integer_quant else None
        self.encoder, self.decoder = models.mnist_encoder(ch), models.mnist_decoder(ch)
        ...
        if self.integer_quant:
            ...
            self.prior_model = models.EntropyModelMnist(self.encoder_channels)
    ...
    def encode(self, x, ch_fraction=1):
        ...
        if self.no_quant:
            return latent, None, torch.ones_like(latent)
    ...
    def example_input_array(self):
        return torch.zeros(1, 1, 28, 28, device=self.device)
